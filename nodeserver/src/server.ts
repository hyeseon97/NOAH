...
import cors from "cors"; // cors를 import합니다.
    ...
    try {
      const uploadPromises = (req.files as Express.Multer.File[]).map(
        async (file) => {
          ...
          image.original_url = compressedImagePath;
...
app.post("/image/get", async (req: Request, res: Response) => {
    ...
    const presignedUrls = await Promise.all(
      images.map(async (image) => {
        const command = new GetObjectCommand({
          Bucket: process.env.AWS_BUCKET_NAME,
          Key: image.original_url, // 이 경우 image_url이 S3 객체의 Key와 동일해야 함
        });
